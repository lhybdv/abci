...
func (cli *grpcClient) SetValidatorsAsync(validators []*types.Validator) *ReqRes {
	req := types.ToRequestSetValidatorsAsync(validators)
	res, err := cli.client.SetValidators(context.Background(), req.GetSetValidators(), grpc.FailFast(true))
	if err != nil {
		cli.StopForError(err)
	}
	return cli.finishAsyncCall(req, &types.Response{&types.Response_SetValidators{res}})
}
